...
class WebSocketEventListener(
    ...
    private fun getSubscriptions(sessionId: String): Set<SimpSubscription>? {
        val sessions: List<SimpSession> = simpUserRegistry.users
    ...
    fun secure(
      ...
      principal: Principal
        ...
        if (message.text.startsWith("/")) {
            ...
            messaging.convertAndSend("/topic/$room", message)
        }
    }

    private fun handleCommand(command: String, principal: StompPrincipal, room: String? = null) {
        ...
        when {
            command.startsWith("/subscribers") -> {
                val subscribers = simpUserRegistry.users
                    .flatMap { user ->
                ...
                messaging.convertAndSendToUser(
                    principal.name,
                    "/queue/command",
                    "/subscribers\nSubscribers in $room: ${subscribers.joinToString(", ")}"
                )
            }
